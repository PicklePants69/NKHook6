...
namespace NKHook6.Api.Utilities
{
    ...
    internal class SelfCheckingDictionary<K, V> : IDictionary<K, V>
    {
        ...
        private void DupFoundAllDiscardedLog(K key)
        {
            ...
            Logger.Log($"{CollectionName} already exists for '{keyLogString}'.{Environment.NewLine}" +
                        $"All entries will be removed so conflict can be noted and resolved.{Environment.NewLine}" +
                        $"So far we have discarded or overwritten {DuplicatesDiscarded[key]} entries for '{keyLogString}'.", Logger.Level.Warning);
        }
