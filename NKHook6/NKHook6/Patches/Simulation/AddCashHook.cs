namespace NKHook6.Patches._Simulation
{
	...
	class AddCashHook
	{
		...
		internal static bool Prefix(ref Simulation __instance, ref double c, ref Simulation.CashType from, ref int cashIndex, ref Simulation.CashSource source, [Optional]ref Tower tower)
		{
			...
			prefixSkipCount++;
			if (c == 0 || (prefixSkipCount < skipNum))
				return true;
			prefixSkipCount = 0;
			...
			if (sendPrefixEvent)
			{
				var o = new SimulationEvents.AddCashEvent.Pre(ref __instance, ref c, ref from, ref cashIndex, ref source, ref tower);
		...
		[HarmonyPostfix]
		internal static void Postfix(ref Simulation __instance, ref double c, ref Simulation.CashType from, ref int cashIndex, ref Simulation.CashSource source, [Optional]ref Tower tower)
		{
			//Have to do weird skipping because the event normally fires all the time for no reason
			postfixSkipCount++;
			if (c == 0 || (postfixSkipCount < skipNum))
				return;
			postfixSkipCount = 0;
			
			if (sendPostfixEvent)
			{
				var o = new SimulationEvents.AddCashEvent.Post(ref __instance, ref c, ref from, ref cashIndex, ref source, ref tower);
				EventRegistry.subscriber.dispatchEvent(ref o);
				__instance = o.instance;
				c = o.cash;
				from = o.from;
